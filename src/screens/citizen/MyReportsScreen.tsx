...
export function MyReportsScreen() {
    ...
    <View style={styles.card}>
      ...
      {item.age || item.sex ? (
        <Text style={styles.patientMeta}>
          {t.my_reports_patient_label(String(item.age ?? ''))}
          {item.age && item.sex ? ' · ' : ''}
          {item.sex ? item.sex : ''}
        </Text>
      ) : null}
      ...
      <View style={styles.symptomsContainer}>
        <Text style={styles.symptomsLabel}>{t.my_reports_symptoms_label}</Text>
        <Text style={styles.cardSymptoms}>{item.symptoms.join(', ')}</Text>
      </View>

      {item.reasoning ? (
        <View style={styles.reasoningCard}>
          <Text style={styles.reasoningTitle}>{t.my_reports_ai_analysis_title}</Text>
  ...
  return (
    <FlatList
      ...
      ListHeaderComponent={<Text style={styles.title}>{t.my_reports_title}</Text>}
      ListEmptyComponent={<Text style={styles.empty}>{t.my_reports_empty}</Text>}
    />
  );
}
