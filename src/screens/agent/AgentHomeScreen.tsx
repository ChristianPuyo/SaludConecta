...
export function AgentHomeScreen() {
  ...
  const AGENT_TASKS = [
    { id: 't1', desc: t.agent_task1, done: false },
    { id: 't2', desc: t.agent_task2, done: true },
    { id: 't3', desc: t.agent_task3, done: false },
  ];

  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Text style={styles.title}>{t.agent_home_title}</Text>
      <Text style={styles.subtitle}>{t.agent_home_subtitle}</Text>
      ...
      <View style={styles.statsRow}>
        <View style={styles.statCard}>
          ...
          <Text style={styles.statLabel}>{t.agent_stat_visited}</Text>
        </View>
        <View style={styles.statCard}>
          <Ionicons
          ...
          <Text style={styles.statLabel}>{t.agent_stat_pending}</Text>
        </View>
      </View>
      ...
      <View style={styles.offlineBanner}>
        ...
        <Text style={styles.offlineText}>{t.agent_offline_banner}</Text>
      </View>

      {/* Critical Monitoring Group */}
      <Text style={styles.sectionLabel}>{t.agent_critical_section(criticalPatients.length)}</Text>
      {criticalPatients.length > 0 ? (
        ...
        <View style={styles.emptyCard}>
          <Text style={styles.emptyText}>{t.agent_critical_empty}</Text>
        </View>
      )}

      {/* Community Tasks */}
      <Text style={styles.sectionLabel}>{t.agent_tasks_section}</Text>
      ...
      <Pressable style={styles.primaryButton} onPress={() => navigation.navigate('RegisterVisit')}>
        ...
        <Text style={styles.primaryButtonText}>{t.agent_register_btn}</Text>
      </Pressable>
    </ScrollView>
  );
}
